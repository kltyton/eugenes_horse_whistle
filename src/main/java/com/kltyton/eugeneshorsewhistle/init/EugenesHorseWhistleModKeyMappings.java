...
public class EugenesHorseWhistleModKeyMappings {
	...
	public static void load() {
		ClientTickEvents.END_CLIENT_TICK.register((client) -> {
			...
			if (WHISTLEaction == 1) {
				ClientPlayNetworking.send(new ResourceLocation(EugenesHorseWhistleMod.MODID, "whistle"), new WhistleMessage(true, false));
			} else if (WHISTLEaction == 2) {
				ClientPlayNetworking.send(new ResourceLocation(EugenesHorseWhistleMod.MODID, "whistle"), new WhistleMessage(false, true));
			}
		});
	}
}
